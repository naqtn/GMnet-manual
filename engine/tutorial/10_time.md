...
それぞれのプレーヤーが個々の時刻カウンタを持つ事になり、
それはつまり全く同期していないのと同じ事になってしまいます。
...
そして、クライアント側であってもカウンター（訳注：time 変数のこと）をカウントアップするようにします。
つまり**それぞれのプレーヤーのゲームはそれ自身のカウントアップを行い**、
サーバーは**必要ならその時刻を補正**します。

（訳注：つまり「ネットワーク負荷軽減のために頻繁には同期を取らずに、
それぞれで時刻は進めていくのだがずれが無いことを保証するために時々同期を取る」という方式だということです。）
...
残る二つの事は時刻をカウントすることと、それを使って実際に何かすることです。
